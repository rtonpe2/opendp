...
def log_regression_train(queryable, *, input_domain, input_metric, output_measure,
           d_in, d_mids, n, d, clip_norm, learning_rate, l2_penalty):
    ...
    for d_mid in d_mids:
        ...
        clamp = make_np_clamp(gradient.output_domain, gradient.output_metric, clip_norm, 2)
        clamp_domain, clamp_metric = clamp.output_space
        step = dp.binary_search_chain(
            lambda s: gradient >> clamp >> make_private_np_sum(clamp_domain, clamp_metric, output_measure, s),
            d_in=d_in, d_out=d_mid, T=float,
            bounds=(0.0, 1e6)
        )
...
def make_private_logistic_regression(
    ...
    d_in=d_in, d_out=d_out, T=float) #figure out budget per step

    d_mids = [rho_per_step] * n_iters


    return (
        dp.c.make_adaptive_composition(input_domain, input_metric, output_measure, d_in, d_mids=d_mids) 
        >> _new_pure_function(lambda queryable: 
                              log_regression_train(queryable, input_domain=input_domain, 
                                                        input_metric=input_metric, 
                                                        output_measure=output_measure,
                                                        d_in=d_in, d_mids=d_mids, n=n, d=d,
